...
//查找未读邮件
function selectDontViewNotice(){
	$.ajax({
		type:"POST",
		url:"/chinatour/admin/remind/unRead.jhtml",
		dataType:"text",
		success:function(returnResult) {
			var JSONobj = eval('('+ returnResult +')');
			var noticeCount = JSONobj.unReadNum;
			
			//站内信提醒
			if(noticeCount == 0){
				$("#notice_nav").attr("class","");
			}else{
				$("#notice_nav").text(noticeCount);
				$("#notice_nav").attr("class","bubble");
			}
			
			$("#Msg").html();
			var n = 0;
			//未审核账单提醒
			var supCheckNum = JSONobj.supCheckNum;
			if(supCheckNum != 0){
			    n=n+1;
				var html = '<li>'
                           +'     <a href="/chinatour/admin/supplierPrice/agentTourList.jhtml">'
                           +'         <i class=" fa fa-comments"></i><span class="date pull-right">View...</span><span id="supCheckNum"  class="name">您有 未审核 的账单</span>' 
                           +'     </a>'
                           +' </li>'
				$("#Msg").append(html);
				$("#news_nav").text(n);
				$("#news_nav").attr("class","bubble");
			}
			
			//OP被拒绝账单提醒
			var supPriceNum = JSONobj.supPriceNum;
			if(supPriceNum != 0){
			    n=n+1;
				var html = '<li>'
                           +'     <a href="/chinatour/admin/supplierPrice/orderInfoList.jhtml">'
                           +'         <i class=" fa fa-comments"></i><span class="date pull-right">View...</span><span id="supCheckNum"  class="name">您有 被拒绝 的账单</span>' 
                           +'     </a>'
                           +' </li>'
				$("#Msg").append(html);
				$("#news_nav").text(n);
			...
			//订单抵达提醒
			var orderSum = JSONobj.orderSum;
			if(orderSum != 0){
				var beginningDate = JSONobj.beginningDate;
				var endingDate = JSONobj.endingDate;
			    n=n+1;
				var html = '<li>'
                  		   	 +'     <a href="/chinatour/admin/orders/list.jhtml?beginningDate='+beginningDate+'&endingDate='+endingDate+'">'
                           +'         <i class=" fa fa-comments"></i><span class="date pull-right">View...</span><span id="supCheckNum"  class="name">您有 即将发团 的订单</span>' 
                           +'     </a>'
                           +' </li>'
				$("#Msg").append(html);
				$("#news_nav").text(n);
			...
			//客人咨询提示
			var customerConsultSum = JSONobj.customerConsultSum;
			if(customerConsultSum>0){
				n=n+1;
				var html = '<li>'
                           +'     <a href="/chinatour/admin/customer/customerConsultlistForNow.jhtml">'
                           +'         <i class=" fa fa-comments"></i><span class="date pull-right">View...</span><span id="supCheckNum"  class="name">您有需要处理的客人咨询记录</span>' 
                           +'     </a>'
                           +' </li>'
				$("#Msg").append(html);
				$("#news_nav").text(n);
...
$(document).ready(function () {
	...
	$("#searchCustomer").on( 'keyup change', function () {
		customerInfo = $("#searchCustomer").val();
		$("#customerInfoList").empty();
    	$.ajax({
			type: "POST",
			url: "/chinatour/admin/remind/findCustomerList.jhtml?customerInfo="+customerInfo,
			success: function(map){
				str="";
				$.each(map.customerList,function(index,customer){
			 		str+='<div class="customer" style="border-bottom:1px solid #ccc; cursor:pointer;background-color:#2494F2;" onclick="location.href=\'/chinatour/admin/customer/editCustomerWithBasicInfo.jhtml?id='+customer.customerId+'&isNew=1\'">'+
				 			 '<p style="height:25px;line-height:25px;margin:0px;padding:0px;margin-left:10px;"><label><i class="fa fa-user-md"></i>&nbsp;&nbsp;'+customer.lastName+'/'+customer.firstName+customer.middleName+'</p>'+
				 			 '<p style="height:25px;line-height:25px;margin:0px;padding:0px;margin-left:10px;"><label><i class="fa fa-phone"></i>&nbsp;&nbsp;'+customer.tel+'</label></p>'+
				 			 '<p style="height:25px;line-height:25px;margin:0px;padding:0px;margin-left:10px;"><label><i class="fa fa-envelope"></i>&nbsp;&nbsp;'+customer.email+'</label></p>'+
			 			 '</div>';
			 	});
			 	if(customerInfo!=""){
			 		$("#customerInfoList").html(str);
			 	}
			 	customerCount = map.customerCount;
			 	if(customerCount>8){
			 		$("#customerInfoList").append('<div style="border-bottom:1px solid #ccc; cursor:pointer;background-color:#2494F2;text-align:center;"  onclick="location.href=\'/chinatour/admin/customer/findAllCustomerList.jhtml?customerInfo='+customerInfo+'\'">Show more results</div>');
			 	}
			}
		})
	})
