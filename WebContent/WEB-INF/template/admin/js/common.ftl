...
function selectDontViewNotice(){
	$.ajax({
		type:"POST",
		url:"${base}/admin/remind/unRead.jhtml",
		dataType:"text",
		success:function(returnResult) {
			...
			if(supCheckNum != 0){
			    n=n+1;
				var html = '<li>'
                           +'     <a href="${base}/admin/supplierPrice/agentTourList.jhtml">'
			...
			if(supPriceNum != 0){
			    n=n+1;
				var html = '<li>'
                           +'     <a href="${base}/admin/supplierPrice/orderInfoList.jhtml">'
			...
			if(orderSum != 0){
			    ...
			    n=n+1;
				var html = '<li>'
                  		   	 +'     <a href="${base}/admin/orders/list.jhtml?beginningDate='+beginningDate+'&endingDate='+endingDate+'">'
			...
			if(customerConsultSum>0){
				n=n+1;
				var html = '<li>'
                           +'     <a href="${base}/admin/customer/customerConsultlistForNow.jhtml">'
...
$(document).ready(function () {
	...
	$("#searchCustomer").on( 'keyup change', function () {
    	...
    	$.ajax({
			type: "POST",
			url: "${base}/admin/remind/findCustomerList.jhtml?customerInfo="+customerInfo,
			success: function(map){
				str="";
				$.each(map.customerList,function(index,customer){
			 		str+='<div class="customer" style="border-bottom:1px solid #ccc; cursor:pointer;background-color:#2494F2;" onclick="location.href=\'${base}/admin/customer/editCustomerWithBasicInfo.jhtml?id='+customer.customerId+'&isNew=1\'">'+
			 	...
			 	if(customerCount>8){
			 		$("#customerInfoList").append('<div style="border-bottom:1px solid #ccc; cursor:pointer;background-color:#2494F2;text-align:center;"  onclick="location.href=\'${base}/admin/customer/findAllCustomerList.jhtml?customerInfo='+customerInfo+'\'">Show more results</div>');
			 	}
			}
		})
	})
