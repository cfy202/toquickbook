...
[#include "/admin/include/sidebar.ftl"]
    ...
    <div class="container-fluid" id="pcont">
        <div class="page-head">
            ...
            <div class="new" style="width:365px">
            ...
            <div class="row">
    			...
    			<div class="tab-container">
						<ul class="nav nav-tabs">
						  <li class="active"><a href="javascript:void(0);" onclick="local('list.jhtml')" data-toggle="tab">Income</a></li>
									...
									<div class="options" style="margin:10px; padding:5px 0;">
											<div  class="nav-panel">
												...
												<div class="nav-block">
													...
													<div class="block-body default-4-line pull-cursor">
														<div class="params-cont">
															<input type="text" id="search_beginningTime" size="14"  placeholder="Beginning Time." />
															<input type="text" id="search_endingTime" size="14"  placeholder="Ending Time." />
												...
												<div class="nav-block">
													...
													<div class="block-body default-4-line pull-cursor">
														<div class="params-cont">
															<input type="text" id="search_beginningApDate" size="14"  placeholder="Beginning Time." />
															<input type="text" id="search_endingApDate" size="14"  placeholder="Ending Time." />
												...
												<div  class="nav-block">
													...
													<div  class="block-body default-2-line">
														<div  class="params-cont"> 
															<input type="text" size="14" id="search_orderNo" placeholder="Order No." [#if pay.orderNo??]value="${pay.orderNo}"[/#if]/>
															<input type="text" size="14" id="search_tourCode" placeholder="TourCode" [#if pay.tourCode??]value="${pay.tourCode}"[/#if]/>
															<input type="text" size="14" id="search_userName" placeholder="Agent" [#if pay.userName??]value="${pay.userName}"[/#if] />
															<input type="text" size="14" id="search_sum" placeholder="Amount" [#if pay.sum??]value="${pay.sum}"[/#if] />
															<input type="text" size="14" id="search_remark" placeholder="Remark" [#if pay.remark??]value="${pay.remark}"[/#if] />
															<input type="text" size="14" id="search_invoiceNo" placeholder="Reference No." [#if pay.code??]value="${p.code}"[/#if] />
															<input type="text" size="14" id="search_item" placeholder="Payment" [#if pay.item??]value="${pay.item}"[/#if] />
	...
	<div class="md-content">
		...
		<form id="sysForm" method="post" action="[@spring.url '/admin/payCostRecords/edit.jhtml'/]">
			<div class="modal-body form">
				<div class="row no-margin-y">
					<div class="form-group col-md-6 col-sm-6  col-xs-6 no-margin">
						Income Type:<input type="text" name="" readonly class="form-control"  id="itemId">
					</div>
					<div class="form-group col-md-6 col-sm-6  col-xs-6 no-margin">
				...
				<div class="row no-margin-y">
					<div class="form-group col-md-6 col-sm-6  col-xs-6 no-margin">
						Amount:<input type="text" readonly name="" class="form-control" id="sumId">
					</div>
					<div class="form-group col-md-6 col-sm-6  col-xs-6 no-margin">
			...
			<div class="modal-footer">
				...
				<button type="button" id="synButton3" class="btn btn-primary">Not to synchronous</button>
...
<script type="text/javascript">
     $(document).ready(function () {
      ...
      $('input').iCheck({
        ...
        var oTable = $('#datatable2').dataTable({
            ...
            "ajax": {
                url: "[@spring.url '/admin/payCostRecords/list.jhtml'/]",
                type: "POST",
                "data": function ( data ) {
		  ...
		  "fnCreatedRow": function( nRow, aData, iDataIndex ) {
		    	  ...
		    	  var str='';
                 		if(aData.isSuccess==0){
                 			...
                 			str+='<input type="checkbox"  name="records" value="'+aData.id+'" class="icheck" disabled="disabled"/>同步失败</i>';
                 		}else{
                 			str+='<input type="checkbox"  name="records" value="'+aData.id+'" class="icheck" disabled="disabled"/>不同步</i>';
                 		}
		...
		$.ajax({
			   ...
			   success: function(msg){	
			   		var obj=eval(msg);
						...
						//$("#wayId").val(obj['way']);
						var way=obj['way'];
						//$("#ways option").removeAttr("selected");
						$("#ways").find("option").each(function(){
							//alert(way+"----->"+$(this).text().trim());
							if($(this).text().trim()==way){
						...
						//alert($("#ways").find("option:selected").text());
	...
	function chbox(){
		...
		$("#fromCentId").append('<input type="hidden" value="'+remark+'" name="remark"/>');
		$("#fromCentId").append('<input type="hidden" value="'+code+'" name="code"/>');
	...
	function printOrder(){
            ...
            var url="?orderNo="+orderNo+"&tourCode="+tourCode+"&userName="+userName+"&status="+status+"&sum="+sum;
            	url+="&remark="+remark+"&beginningDate="+beginningDate+"&endingDate="+endingDate+"&code="+code+"&item="+item+"&way="+way+"&beginningTime="+beginningTime+"&endingTime="+endingTime;
	    window.open("[@spring.url '/admin/payCostRecords/findOrderListVOPrintOfPay.jhtml'/]"+url);
	};
