...
<script type="text/javascript" src="https://appcenter.intuit.com/Content/IA/intuit.ipp.anywhere-1.3.2.js">
 ...
 intuit.ipp.anywhere.setup({
      menuProxy: '',
      grantUrl: 'http://localhost:8080/toQuickBook/admin/oauth/request_token.jhtml?companyId=123145774690182'
      })
 </script>
 -->
<script type="text/javascript">
     $(document).ready(function () {
        ...
        var oTable = $('#datatable2').dataTable({
            ...
            "ajax": {
                url: "[@spring.url '/admin/vender/customerList.jhtml'/]",
                type: "POST",
                "data": function ( data ) {
           ...
           "columns": [
                { "data": "venderId",
                	"render" : function(data, type, row) {
						if(row.isSuccess==0){	
	                     	return '<div class="radio"><input type="checkbox"  name="customerIds" value="'+data+'" class="icheck"/></div>';
	                     }else{
	                     	return '<div class="radio"><input type="checkbox"  name="customerIds" value="'+data+'" class="icheck" disabled="disabled"/></div>';
	                     }
					}},
                { "data": "name" },
                { "data": "cityId" },
            ...
            "columnDefs" : [ {
				"render" : function(data, type, row) {
						if(row.isSuccess==0){
							return '<a style="cursor:pointer;" href="synchronousCustomer.jhtml?customerIds='+data+'">Synchronous</a>';
						}else{
							return 'Synchronous';
						}
                 },
     ...
     $("#check-all").on('change',function(){
  	  ...
  	  $("#synchronousButton").click(function(){
  	 	...
  	 	if(n==0){
  	 		...
  	 		$("input[name='customerIds']:checkbox:checked").each(function(){
  	 			customerIds += $(this).val()+",";
  	 		});
  	 		customerIds = customerIds.substring(customerIds,customerIds.length-1);
  	 		window.location.href="${base}/admin/vender/synchronousCustomer.jhtml?customerIds="+customerIds;
  	 	}
  	 });	 
</script>
</body>
