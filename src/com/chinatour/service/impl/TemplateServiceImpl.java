...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
    ...
    public List<Template> getAll() {
        try {
            File settingsXmlFile = new ClassPathResource(CommonAttributes.SETTINGS_XML_PATH).getFile();
            Document document = new SAXReader().read(settingsXmlFile);
            List<Template> templates = new ArrayList<Template>();
            List<Element> elements = document.selectNodes("/settings/template");
    ...
    public List<Template> getList(Template.Type type) {
        if (type != null) {
            try {
                ...
                Document document = new SAXReader().read(settingsXmlFile);
                List<Template> templates = new ArrayList<Template>();
                List<Element> elements = document.selectNodes("/settings/template[@type='" + type + "']");
                for (Element element : elements) {
                    Template template = getTemplate(element);
                    templates.add(template);
                }
    ...
    public Template get(String id) {
        try {
            File settingsXmlFile = new ClassPathResource(CommonAttributes.SETTINGS_XML_PATH).getFile();
            Document document = new SAXReader().read(settingsXmlFile);
            Element element = (Element) document.selectSingleNode("/settings/template[@id='" + id + "']");
            Template template = getTemplate(element);
            return template;
        } catch (Exception e) {
            e.printStackTrace();
